package asmock.tests.framework.impl.stub
{
	...
	public class StubRecordMockStateFixture extends ExtendedTestCase
	{
		...
		{
			var state : StubRecordMockState = new StubRecordMockState(_mockObject, _mockRepository);
		...
		{
			var recordState : StubRecordMockState = new StubRecordMockState(_mockObject, _mockRepository);
		...
		public function testReplay_lastMethodClosed() : void
		{
			...
			assertTrue("Unexpected result from replay()", state is StubReplayMockState);
		}
		...
		public function test_backToRecord_returnsClonedStubRecordMockState() : void
		{
			var recordState : StubRecordMockState = new StubRecordMockState(_mockObject, _mockRepository);
			
			var newMockState : IMockState = recordState.backToRecord();
			
			assertTrue("Expected new instance from backToRecord", newMockState is StubRecordMockState);
			assertNotStrictlyEquals(recordState, newMockState, "Expected new instance from backToRecord");
			
			assertStrictlyEquals("Unexpected proxy value on new instance", _mockObject, (newMockState as StubRecordMockState).proxy);
			assertStrictlyEquals("Unexpected lastExpectation value on new instance", _mockRepository, (newMockState as StubRecordMockState).repository);
			assertNull("Unexpected lastExpectation value on new instance", (newMockState as StubRecordMockState).lastExpectation);
		}
		
		private function assertExpectationNotClosedError(state : StubRecordMockState, func : Function) : void
		{
