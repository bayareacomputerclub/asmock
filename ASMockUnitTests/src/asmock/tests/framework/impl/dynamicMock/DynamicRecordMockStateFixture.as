package asmock.tests.framework.impl.dynamicMock
{
	...
	public class DynamicRecordMockStateFixture extends ExtendedTestCase
	{
		private var _mockObject : Object = new Object();
		private var _mockRepository : MockMockRepository = new MockMockRepository();
		
		public function DynamicRecordMockStateFixture()
		{
			super();
		}
		...
		{
			var state : DynamicRecordMockState = new DynamicRecordMockState(_mockObject, _mockRepository);
		...
		{
			var recordState : DynamicRecordMockState = new DynamicRecordMockState(_mockObject, _mockRepository);
		...
		public function testReplay_lastMethodClosed() : void
		{
			...
			assertTrue("Unexpected result from replay()", state is DynamicReplayMockState);
		}
		...
		public function test_backToRecord_returnsClonedDynamicRecordMockState() : void
		{
			var recordState : DynamicRecordMockState = new DynamicRecordMockState(_mockObject, _mockRepository);
			
			var newMockState : IMockState = recordState.backToRecord();
			
			assertTrue("Expected new instance from backToRecord", newMockState is DynamicRecordMockState);
			assertNotStrictlyEquals(recordState, newMockState, "Expected new instance from backToRecord");
			
			assertStrictlyEquals("Unexpected proxy value on new instance", _mockObject, (newMockState as DynamicRecordMockState).proxy);
			assertStrictlyEquals("Unexpected lastExpectation value on new instance", _mockRepository, (newMockState as DynamicRecordMockState).repository);
			assertNull("Unexpected lastExpectation value on new instance", (newMockState as DynamicRecordMockState).lastExpectation);
		}
		
		private function assertExpectationNotClosedError(state : DynamicRecordMockState, func : Function) : void
		{
